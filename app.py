...
# YOUTUBE_API_KEY = "AIzaSyCkTzZjYyrTgKhTRX-bmfipYJQIVr8Cn4k"
...
# Set your YouTube Data API key here
YOUTUBE_API_KEY = "AIzaSyBfINzC388eBJ3Uca4KXMTnxuay57kuDNI"
...
def get_channel_analytics(channel_id):
    response = fetch_youtube_data(
        youtube.channels().list(part="snippet,statistics", id=channel_id)
    )
...
def get_all_video_details(channel_id):
    response = fetch_youtube_data(
        youtube.search().list(
            channelId=channel_id, type="video", part="id,snippet", maxResults=50
        )
    )
    ...
    if response and "items" in response:
        ...
        for item in response.get("items", []):
            ...
            video_info = fetch_youtube_data(
                youtube.videos().list(part="statistics,snippet", id=video_id)
            )

            if video_info and "items" in video_info:
                statistics_info = video_info["items"][0]["statistics"]
                snippet_info = video_info["items"][0]["snippet"]
                ...
                video_details.append(
                    (title, video_id, likes, views, comments, upload_date, channel_name, url)
                )

        return pd.DataFrame(
            video_details,
            columns=["Title", "Video ID", "Likes", "Views", "Comments", "Upload Date", "Channel", "URL"],
        )

    return pd.DataFrame(columns=["Title", "Video ID", "Likes", "Views", "Comments", "Upload Date", "Channel", "URL"])

# Function to get video recommendations based on topic
def get_video_recommendations(topic, max_results=10):
    response = fetch_youtube_data(
        youtube.search().list(
            q=topic, type="video", part="id,snippet", maxResults=max_results, order="viewCount"
        )
    )
    ...
    if response and "items" in response:
        ...
        for item in response.get("items", []):
            ...
            if video_info and "items" in video_info:
                ...
                video_details.append((title, video_id, views, channel_name, url, comments))
...
def analyze_and_categorize_comments(comments):
    ...
    for comment in comments:
        ...
        if polarity > 0:
            categorized_comments["Positive"].append((comment, polarity))
        elif polarity == 0:
            categorized_comments["Neutral"].append((comment, polarity))
        else:
            categorized_comments["Negative"].append((comment, polarity))

    return categorized_comments

# Main Streamlit app
st.title("YouTube Analyzer")
st.markdown("""Explore detailed analytics, recommendations, and comment sentiments for YouTube videos and channels.""")

# Sidebar
st.sidebar.title("Navigation")
option = st.sidebar.radio("Choose a task", ["Channel Analytics", "Video Recommendations", "Sentiment Analysis"])

if option == "Channel Analytics":
    channel_id = st.text_input("Enter Channel ID")

    if st.button("Get Analytics"):
        channel_title, description, country, total_videos, total_views, videos_df = get_channel_analytics(channel_id)
        ...
        if channel_title:
            ...
            fig = px.bar(videos_df, x="Title", y="Views", title="Video Views")
            st.plotly_chart(fig)

if option == "Video Recommendations":
    topic = st.text_input("Enter Topic")

    if st.button("Get Recommendations"):
        recommendations = get_video_recommendations(topic)
        ...
        if recommendations:
            for video in recommendations:
                ...
                st.write(f"Views: {video[2]}")
                st.write(f"Channel: {video[3]}")
                st.write(f"[Watch Video]({video[4]})")
...
if option == "Sentiment Analysis":
    ...
    if st.button("Analyze Sentiments"):
        ...
        if comments:
            sentiments = analyze_and_categorize_comments(comments)

            st.subheader("Sentiment Analysis")
            for sentiment, sentiment_comments in sentiments.items():
                st.write(f"**{sentiment}: {len(sentiment_comments)} comments**")
